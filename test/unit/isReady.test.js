...
const {
  CQWebSocketAPI: { WebSocketType, CQWebSocket },
...
test.cb('#isReady(): event-enabled, api-enabled, event-connected, api-connected', function (t) {
  ...
  const bot = new CQWebSocket().on('socket.connect', _stub).connect();
...
test.cb('#isReady(): event-enabled, api-enabled, api-connected', function (t) {
  ...
  const bot = new CQWebSocket().on('socket.connect', _stub).connect(WebSocketType.API);
