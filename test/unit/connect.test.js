...
test('#connect()', function (t) {
  ...
  const _spy = spy();
  const bot = new CQWebSocket().on('socket.connecting', _spy).connect();
...
test('#connect(wsType="/event")', function (t) {
  ...
  const _spy = spy();
  const bot = new CQWebSocket().on('socket.connecting', _spy).connect(WebSocketType.EVENT);
  t.is(bot._monitor.EVENT.state, WebSocketState.CONNECTING);
...
test('#connect(wsType="/api")', function (t) {
  ...
  const _spy = spy();
  const bot = new CQWebSocket().on('socket.connecting', _spy).connect(WebSocketType.API);
