...
const {
  CQWebSocketAPI: { CQWebSocket, CQAt, APITimeoutError },
} = require('../fixture/connect-success')();
...
test.cb('CQEvent: can decide which type of messages to append on according to the format', function (t) {
  ...
  t.context.bot
    ...
    .on('message.private', function (e) {
      const msg = e.messageFormat === 'array' ? { type: 'text', data: { text: 'hello' } } : 'hello';
      e.appendMessage(msg);
    })
    .on('message.private', function (e) {
...
test.cb('CQEvent: listen for response result on the CQEvent', function (t) {
  ...
  t.context.bot.on('message.private', function (e) {
...
test.cb('CQEvent: can work without response handler', function (t) {
  ...
  t.context.bot.on('message.private', function () {
    setTimeout(() => {
      t.end();
    }, 1000);
    return 'some messages';
...
test.cb('CQEvent: listen for response error on the CQEvent', function (t) {
  ...
  t.context.bot.on('error', errorSpy).on('message.private', function (e) {
    e.onResponse({
      timeout: 1000,
    });
